...
public class WorldGuardCommands {
    ...
    public void version(CommandContext args, Actor sender) throws CommandException {
        WorldGuard.getInstance().getMessageService().send(sender, "commands.version-name",
                MessageContext.of("version", WorldGuard.getVersion()));
        WorldGuard.getInstance().getMessageService().send(sender, "commands.version-url");
    ...
    public void reload(CommandContext args, Actor sender) throws CommandException {
        ...
        if (!tasks.isEmpty()) {
            throw WorldGuard.getInstance().getMessageService().commandException("commands.reload-pending-tasks");
        }
        ...
        try {
            ...
            // WGBukkit.cleanCache();
            WorldGuard.getInstance().getMessageService().send(sender, "commands.reload-success");
        } catch (Throwable t) {
            WorldGuard.getInstance().getMessageService().send(sender, "commands.reload-error",
                    MessageContext.of("error", t.getMessage()));
        } finally {
    ...
    public void report(CommandContext args, final Actor sender) throws CommandException, AuthorizationException {
        ...
        try {
            ...
            WorldGuard.getInstance().getMessageService().send(sender, "commands.report-written",
                    MessageContext.of("file", dest.getAbsolutePath()));
        } catch (IOException e) {
            throw WorldGuard.getInstance().getMessageService().commandException("commands.report-write-failed",
                    MessageContext.of("error", e.getMessage()));
        }
    ...
    public void profile(final CommandContext args, final Actor sender) throws CommandException, AuthorizationException {
        ...
        if (args.argsLength() == 0) {
            ...
            if (minutes < 1) {
                throw WorldGuard.getInstance().getMessageService().commandException("commands.profile-min-minutes");
            } else if (minutes > 10) {
                throw WorldGuard.getInstance().getMessageService().commandException("commands.profile-max-minutes");
            }
        }
        ...
        if (args.hasFlag('i')) {
            ...
            if (interval < 1 || interval > 100) {
                throw WorldGuard.getInstance().getMessageService().commandException("commands.profile-interval-range");
            }
            if (interval < 10) {
                WorldGuard.getInstance().getMessageService().send(sender, "commands.profile-low-interval");
            }
        }
        Sampler sampler;
        ...
        synchronized (this) {
            if (activeSampler != null) {
                throw WorldGuard.getInstance().getMessageService().commandException("commands.profile-active");
            }
        ...
        WorldGuard.getInstance().getMessageService().send(sender, "commands.profile-start",
                MessageContext.of("minutes", minutes));
        ...
        Futures.addCallback(sampler.getFuture(), new FutureCallback<>() {
            ...
            public void onSuccess(Sampler result) {
                ...
                try {
                    ...
                    WorldGuard.getInstance().getMessageService().send(sender, "commands.profile-written",
                            MessageContext.of("file", dest.getAbsolutePath()));
                } catch (IOException e) {
                    WorldGuard.getInstance().getMessageService().send(sender, "commands.profile-write-failed",
                            MessageContext.of("error", e.getMessage()));
                }
    ...
    public void stopProfile(CommandContext args, final Actor sender) throws CommandException {
        synchronized (this) {
            if (activeSampler == null) {
                throw WorldGuard.getInstance().getMessageService().commandException("commands.profile-none");
            }
        ...
        WorldGuard.getInstance().getMessageService().send(sender, "commands.profile-cancelled");
    }
    ...
    public void flushStates(CommandContext args, Actor sender) throws CommandException {
        if (args.argsLength() == 0) {
            ...
            WorldGuard.getInstance().getMessageService().send(sender, "commands.flush-all");
        } else {
            ...
            if (player != null) {
                ...
                WorldGuard.getInstance().getMessageService().send(sender, "commands.flush-player",
                        MessageContext.of("player", player.getName()));
            }
        }
    }
    ...
    public void listRunningTasks(CommandContext args, Actor sender) throws CommandException {
        ...
        if (tasks.isEmpty()) {
            WorldGuard.getInstance().getMessageService().send(sender, "commands.running-none");
        } else {
            tasks.sort(new TaskStateComparator());
            MessageBox builder = new MessageBox(WorldGuard.getInstance().getMessageService().plain("commands.running-title"),
                    new TextComponentProducer());
            builder.append(WorldGuard.getInstance().getMessageService().component("commands.running-note"));
            for (Task<?> task : tasks) {
