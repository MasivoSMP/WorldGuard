    ...
    public void onProjectile(DamageEntityEvent event) {
        ...
        if (blockedEffect != null) {
            ...
            if (player != null) {
                ...
                getPlugin().sendMessage(player, "blocked-potions.arrow", MessageContext.of("effect", blockedEffect.getName()));
            }
                ...
                if (player != null) {
                    if (getPlugin().hasPermission(player, "worldguard.override.potions")) {
                        if (wcfg.blockPotionsAlways && (item.getType() == Material.SPLASH_POTION
                                || item.getType() == Material.LINGERING_POTION)) {
                            getPlugin().sendMessage(player, "blocked-potions.bypass-throw",
                                    MessageContext.of("effect", blockedEffect.getName()));
                            event.setCancelled(true);
                        }
                    } else {
                        getPlugin().sendMessage(player, "blocked-potions.potion", MessageContext.of("effect", blockedEffect.getName()));
                        event.setCancelled(true);
                    }
                } else {
                    event.setCancelled(true);
